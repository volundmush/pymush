...
def render_select_screen(connection):
    ...
    if acc.connections:
        ...
        for c in acc.connections:
            t2.add_row(
                f"{c.client_id}",
                f"{str(c.details.protocol)}",
                f"{c.details.host_address}",
                "",
                f"{c.details.client_name}",
                f"{c.details.width}",
            )
        out.add(t2)
        ...
        for objid, c in chars.items():
            t3.add_row(
                ...
                send_menu(
                    c.name,
                    (
                        (f"@ic {c.name}", f"Join the game as {c.name}"),
                        (f"@examine {c.name}", f"@examine {c.name}"),
                    ),
                ),
            )
        out.add(t3)
    ...
    t4.add_row(
        ansify(cmd_style, "@chardelete <name>=<password>"), "Delete a character."
    )
