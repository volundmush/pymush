...
class BanCommand(MushCommand):
    ...
    def execute(self):
        ...
        if seconds > 0:
            ...
            data = {
                "until": banned_until.timestamp(),
                "by": self.enactor.objid,
                "on": now.timestamp(),
            }
...
class ListCommand(MushCommand):
    ...
    def execute(self):
        ...
        options = {"accounts": self.list_accounts, "districts": self.list_districts}
    ...
    def list_accounts(self):
        ...
        for acc in accounts:
            table.add_row(
                acc.objid, acc.name, ", ".join(x.name for x in acc.characters.all())
            )
        out.add(table)
        out.add(fmt.Footer())
    ...
    def list_districts(self):
        ...
        if not (districts := [d for d in districts if not d.relations.get("parent")]):
            ...
            raise CommandException(
                "There are no districts to list. This probably shouldn't happen - contact devs"
            )
        out = fmt.FormatList(self.enactor)
...
class WhoCommand(Command):
    ...
    def who_accounts(self):
            ...
            accounts := list(
                set(
                    [
                        acc
                        for c in t.objects
                        if (acc := c.relations.get("account", None))
                    ]
                )
            )
        ):
        ...
        for acc in accounts:
            ...
            t1.add_row(
                acc.name,
                AnsiString.from_args(
                    red_yellow_green(idle_ryg), duration_format(idle, width=4)
                ),
